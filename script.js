...
const menuStatics = [
    Bodies.rectangle(Game.width / 2, Game.height * 0.4, 512, 512, {
        isStatic: true,
        render: { sprite: { texture: './assets/lmfao.png' } },
    }),
    ...
    Bodies.rectangle(Game.width / 2, Game.height * 0.75, 512, 96, {
        isStatic: true,
        label: 'btn-start',
        render: { sprite: { texture: './assets/start.png' } },
    }),
];
...
document.getElementById("game-next-fruit").onclick = function () {
    // Update currentFruitSize to a random size between 0 and 4
    Game.currentFruitSize = Math.floor(Math.random() * 5);
    console.log("Current Fruit Size:", Game.currentFruitSize);
}
